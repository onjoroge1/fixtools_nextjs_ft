...
import { deletePDFPages, getDeletePagesStats, parsePageRange } from '../../../lib/pdf-server/converters/delete-pdf-pages';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pagesToDelete = fields.pagesToDelete?.[0]; // Page range string (e.g., "1-3,5,7-9")
    filesData = files;
    ...
    if (!pagesToDelete || !pagesToDelete.trim()) {
      throw new ValidationError('No pages specified for deletion. Please provide page numbers or ranges to delete (e.g., "1-3,5,7-9").', 'pagesToDelete');
    }
    ...
    const processFile = async (file) => {
      ...
      try {
        parsePageRange(pagesToDelete, pageCount);
      } catch (parseError) {
        ...
        throw new ValidationError(errorMessage, 'pagesToDelete');
      }

      // Delete pages (pass string, function will parse internally)
      let modifiedPdfBuffer;
      try {
        modifiedPdfBuffer = await deletePDFPages(pdfBuffer, pagesToDelete);
      } catch (deleteError) {
        // Convert ConversionError to ValidationError for consistent API responses
        if (deleteError instanceof ConversionError || deleteError.message) {
          throw new ValidationError(deleteError.message || 'Failed to delete pages from PDF.', 'pagesToDelete');
        }
        throw deleteError;
      }

      // Get statistics
      const stats = getDeletePagesStats(pageCount, pagesToDelete);

      const outputFilename = generateOutputFilename(
        file.originalFilename || 'document.pdf',
        '.pdf'
      ).replace('.pdf', '-pages-deleted.pdf');
    ...
    if (Array.isArray(pdfFiles)) {
      ...
      return res.status(200).json({
        ...
        message: `Pages deleted from ${pdfFilesArray.length} PDF document(s) successfully`,
      });
    } else {
      // Single file
      ...
      return res.status(200).json({
        ...
        message: 'Pages deleted from PDF successfully',
      });
    }
  } catch (error) {
