/**
 * Excel to PDF Conversion API Route
 * Converts Excel spreadsheets to PDF format
 * 
 * POST /api/pdf/excel-to-pdf
 * Body: FormData with 'excel' file(s) for batch processing
...
import { convertExcelToPDF as convertExcel } from '../../../lib/pdf-server/converters/excel-to-pdf';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!excelFiles || excelFiles.length === 0) {
      throw new ValidationError('No Excel file provided', 'excel');
    }

    // Validate file types
    for (const file of excelFiles) {
      if (!validateFileType(file.originalFilename || '', ['.xlsx', '.xls'])) {
        throw new ValidationError('Invalid file type. Please upload Excel files (.xlsx or .xls).', 'excel');
      }
    }
    ...
    if (fileCount === 1) {
      ...
      if (!fs.existsSync(excelFile.filepath)) {
        throw new ValidationError('Uploaded file not found. Please try again.', 'excel');
      }

      const excelBuffer = fs.readFileSync(excelFile.filepath);
      const pdfBuffer = await convertExcel(excelBuffer);

      const outputFilename = generateOutputFilename(
        excelFile.originalFilename || 'spreadsheet.xlsx',
        '.pdf'
      );

      // Clean up temp file after successful conversion
      cleanupFiles(excelFile.filepath);
      ...
      // Batch processing - convert all files and return as ZIP or array
      const convertedFiles = [];
      
      for (const excelFile of excelFiles) {
        if (!fs.existsSync(excelFile.filepath)) {
          console.warn(`File not found: ${excelFile.filepath}`);
          continue;
        }
        ...
        try {
          ...
          console.error(`Error converting ${excelFile.originalFilename}:`, fileError);
          // Continue with other files
        }
      }

      if (convertedFiles.length === 0) {
        throw new ValidationError('Failed to convert any files. Please check your Excel files and try again.', 'excel');
      }

      return res.status(200).json({
        success: true,
        files: convertedFiles,
        message: `Successfully converted ${convertedFiles.length} of ${fileCount} Excel files to PDF`,
    ...
    if (filesData && filesData.excel) {
      filesData.excel.forEach(file => {
