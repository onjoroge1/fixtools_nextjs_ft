...
export default function ExtractPDFPages() {
  ...
  const canonicalUrl = `${siteHost}/pdf/extract-pdf-pages`;
  ...
  const extractPages = async () => {
    ...
    if (!pagesToExtract || !pagesToExtract.trim()) {
      setError('Please specify which pages to extract (e.g., "1-3,5,7-9").');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/pdf/extract-pdf-pages', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || data.message || 'Failed to extract pages from PDF');
      }
      ...
      setError('Failed to extract pages: ' + (err.message || 'Unknown error'));
    } finally {
      setIsProcessing(false);
  ...
  const handleDownload = (fileData, index) => {
    if (!fileData || !fileData.file) {
      setError('No PDF to download. Please extract pages first.');
      return;
    }

    const link = document.createElement('a');
    link.href = `data:application/pdf;base64,${fileData.file}`;
    link.download = fileData.filename || `pdf-extracted-${index + 1}.pdf`;
    link.click();
  };
  ...
  const pageRangeValidation = validatePageRange(pagesToExtract);
  ...
  const structuredData = {
      ...
      "mainEntity": [
        {
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Upload your PDF file using the upload button or drag and drop it into the designated area. Enter the page numbers or ranges you want to extract (e.g., '1-3,5,7-9' to extract pages 1, 2, 3, 5, 7, 8, and 9). Click 'Extract Pages' to process your file. The tool will create a new PDF with the specified pages extracted. Once processing is complete, download your extracted PDF file."
          }
        },
        {
          "@type": "Question",
          "name": "Is extracting PDF pages free?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, our PDF page extraction tool is free for single PDF files up to 10MB. For larger files (above 10MB) or batch processing (multiple files), a Processing Pass is required. All processing happens on our secure servers, ensuring your PDF files are processed quickly and efficiently."
          }
        },
        {
          "@type": "Question",
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Our tool supports flexible page range formats. You can specify individual pages separated by commas (e.g., '1,3,5'), page ranges using hyphens (e.g., '1-3'), or a combination of both (e.g., '1-3,5,7-9'). The tool will extract all specified pages and create a new PDF containing only those pages."
          }
        },
        {
          "@type": "Question",
          "name": "Can I extract pages from multiple PDFs at once?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes! You can upload multiple PDF files at once and the same page range will be applied to all files. Each PDF will be processed separately, and you'll receive individual files with pages extracted. For batch processing (2+ files), a Processing Pass is required for free tier users. Paid users can process up to 20 files in a single batch."
          }
        },
        {
          "@type": "Question",
          "name": "Is my PDF data secure when extracting pages?",
          "acceptedAnswer": {
            "@type": "Answer",
        ...
        {
          "@type": "Question",
          "name": "Can I extract all pages from a PDF?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, you can extract all pages from a PDF by specifying the full page range (e.g., '1-30' for a 30-page PDF). The tool will create a new PDF containing all the specified pages. This is useful for creating copies or reorganizing PDF documents."
          }
        },
        {
          "@type": "Question",
          "name": "Will the extracted pages maintain their quality?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, all extracted pages maintain the original quality of the source PDF. No compression or quality loss occurs during page extraction. All text, images, graphics, and formatting from the original pages are preserved in the resulting PDF document."
          }
        }
      ]
    },
    softwareApp: {
      ...
      "name": "Extract PDF Pages - Extract Pages from PDF Online",
      "applicationCategory": "DeveloperApplication",
      "operatingSystem": "Any",
      "description": "Free online tool to extract pages from PDF files. Create new PDFs from selected pages or page ranges. Batch processing supported. Works securely on our servers with automatic file cleanup.",
      "offers": {
        "@type": "Offer",
        "price": "0",
        "priceCurrency": "USD"
    ...
    howTo: {
      ...
      "description": "Step-by-step guide to extract pages from PDF files online.",
      "step": [
        {
          "@type": "HowToStep",
        ...
        },
        {
          ...
          "text": "Enter the page numbers or ranges you want to extract. Use individual pages (e.g., '1,3,5'), page ranges (e.g., '1-3'), or a combination (e.g., '1-3,5,7-9'). The tool will validate your input and show a preview of the pages to be extracted.",
          "position": 2
        },
        {
          "@type": "HowToStep",
          "name": "Extract pages and download",
          "text": "Click the 'Extract Pages' button to process your file. The tool will create a new PDF with the specified pages extracted. Once processing is complete, click 'Download PDF' to save your extracted PDF file. For batch processing, download individual files or all files at once.",
          "position": 3
        }
      ]
    },
  ...
  return (
    <>
      <Head>
        ...
        <title>Extract PDF Pages - Free Online Tool to Extract Pages from PDF | FixTools</title>
        <meta name="title" content="Extract PDF Pages - Free Online Tool to Extract Pages from PDF | FixTools" />
        <meta name="description" content="Extract pages from PDF files online for free. Create new PDFs from selected pages or page ranges. Batch processing supported. Works securely on our servers - fast, reliable, no registration required." />
        <meta name="keywords" content="extract pdf pages, extract pages from pdf, extract pdf pages online, pdf page extractor, extract specific pdf pages, extract pages from pdf file, extract pdf pages online, pdf page extraction tool" />
        ...
        <meta property="og:title" content="Extract PDF Pages - Free Online Tool to Extract Pages from PDF" />
        <meta property="og:description" content="Extract pages from PDF files online for free. Create new PDFs from selected pages or page ranges. Batch processing supported." />
        <meta property="og:image" content={`${siteHost}/images/og-extract-pdf-pages.png`} />
        <meta property="og:site_name" content="FixTools" />
        
        {/* Twitter */}
        <meta property="twitter:card" content="summary_large_image" />
        <meta property="twitter:url" content={canonicalUrl} />
        <meta property="twitter:title" content="Extract PDF Pages - Free Online Tool to Extract Pages from PDF" />
        <meta property="twitter:description" content="Extract pages from PDF files online for free. Create new PDFs from selected pages or page ranges." />
        <meta property="twitter:image" content={`${siteHost}/images/og-extract-pdf-pages.png`} />
      ...
      <style jsx global>{`
        html:has(.extract-pdf-pages-page) {
        ...
        .extract-pdf-pages-page *,
        .extract-pdf-pages-page *::before,
        .extract-pdf-pages-page *::after {
          box-sizing: border-box;
        }
        
        .extract-pdf-pages-page h1,
        .extract-pdf-pages-page h2,
        .extract-pdf-pages-page h3,
        .extract-pdf-pages-page p,
        .extract-pdf-pages-page ul,
        .extract-pdf-pages-page ol {
          margin: 0;
        }
        
        .extract-pdf-pages-page button {
          font-family: inherit;
          cursor: pointer;
        }
        
        .extract-pdf-pages-page input,
        .extract-pdf-pages-page textarea,
        .extract-pdf-pages-page select {
          font-family: inherit;
        }
      `}</style>

      <div className="extract-pdf-pages-page bg-[#fbfbfc] text-slate-900 min-h-screen">
        {/* Header */}
        ...
        <nav className="mx-auto max-w-6xl px-4 py-3" aria-label="Breadcrumb">
          <ol className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <li className="flex items-center gap-2">
              <span className="text-slate-400">/</span>
              <span className="font-semibold text-slate-900">Extract PDF Pages</span>
            </li>
          </ol>
        </nav>
          ...
          <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 py-10 md:grid-cols-12 md:py-14">
            ...
            <div className="relative z-10 md:col-span-7 hero-content">
              ...
              <p className="mt-4 max-w-xl text-base leading-relaxed text-slate-600 md:text-lg md:leading-relaxed">
                Our <strong>Extract PDF Pages</strong> tool helps you extract specific pages or page ranges from PDF documents instantly. Create new PDFs from selected pages. Batch processing supported for multiple files. Works securely on our servers - fast, reliable, no registration required.
              </p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-slate-600 leading-relaxed">
                        Extract specific pages (e.g., '1,3,5'), page ranges (e.g., '1-3'), or combinations (e.g., '1-3,5,7-9'). The tool validates your input and processes pages accurately.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="feature-card group rounded-3xl border-2 border-slate-200 bg-gradient-to-br from-white to-slate-50 p-6 shadow-lg transition-all duration-300 hover:border-blue-300 hover:shadow-xl hover:-translate-y-1">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-slate-600 leading-relaxed">
                        Preview your PDF before extracting pages. Navigate through pages to verify which pages you want to extract, ensuring accuracy in your page selection.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="feature-card group rounded-3xl border-2 border-slate-200 bg-gradient-to-br from-white to-slate-50 p-6 shadow-lg transition-all duration-300 hover:border-purple-300 hover:shadow-xl hover:-translate-y-1">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-sm text-slate-600 leading-relaxed">
                        All page extraction happens on secure servers. Files are automatically deleted after processing to ensure your data remains private and secure.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <section id="tool" className="mx-auto max-w-6xl px-4 pb-12 pt-2">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 md:p-7" style={{ boxShadow: '0 12px 40px rgba(2, 6, 23, 0.08)' }}>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">Extract Pages from PDF</h2>
                <p className="mt-1 text-sm text-slate-600">Upload PDF file(s) and specify which pages to extract</p>
              </div>
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  className={`w-full rounded-xl border-2 ${
                    pagesToExtract && !pageRangeValidation.valid 
            ...
            {/* Extract Button */}
            {pdfFiles.length > 0 && pagesToExtract && pageRangeValidation.valid && !isProcessing && (
              <div className="mb-6">
                <button
                ...
                >
                  {isProcessing ? 'Extracting Pages...' : `⚡ Extract Pages from ${pdfFiles.length} PDF${pdfFiles.length !== 1 ? 's' : ''}`}
                </button>
              </div>
            )}
            ...
            {isProcessing && (
              <div className="mb-6 rounded-lg border-2 border-blue-200 bg-blue-50 p-4">
                <div className="flex items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-semibold text-blue-900">Extracting pages from PDF files...</p>
                </div>
                {progress.total > 0 && (
                  <div className="mt-3">
                        ...
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-slate-900 truncate" title={fileData.filename}>
                              {fileData.filename || `pdf-extracted-${index + 1}.pdf`}
                            </p>
                            {fileData.stats && (
                              <p className="text-xs text-slate-500 mt-1">
                                {fileData.stats.totalPages} pages → {fileData.stats.pagesExtracted} pages extracted ({fileData.stats.extractionPercentage}%)
                              </p>
                            )}
                          </div>
                        </div>
        ...
        {/* What is Extract PDF Pages Section */}
        <section className="mx-auto max-w-6xl px-4 pb-12">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-10" style={{ boxShadow: '0 12px 40px rgba(2, 6, 23, 0.08)' }}>
            
            <div className="inline-flex items-center gap-2 mb-4">
              <div className="h-1 w-8 bg-gradient-to-r from-emerald-500 to-green-500 rounded-full"></div>
              <h2 className="text-3xl font-bold text-slate-900">What is Extract PDF Pages?</h2>
            </div>
            
            <div className="prose prose-slate max-w-none">
              <p className="text-base text-slate-700 leading-relaxed mb-4">
                <strong>Extract PDF Pages</strong> is the process of extracting specific pages or page ranges from PDF documents. This is useful for cleaning up PDF files, extracting selected pages, extracting specific content, creating custom PDF documents, and reducing file size by extracting specific pages.
              </p>
              
              <p className="text-base text-slate-700 leading-relaxed mb-4">
                According to <a href="https://pdf-lib.js.org/" target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:text-emerald-800 font-semibold underline">pdf-lib documentation</a>, PDF page extraction is accomplished by creating a new PDF document and copying all pages to be extracted. The <a href="https://pdf-lib.js.org/" target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:text-emerald-800 font-semibold underline">pdf-lib library</a> provides comprehensive PDF manipulation capabilities that allow precise control over page selection and deletion.
              </p>

              <p className="text-base text-slate-700 leading-relaxed mb-4">
                Extracting PDF pages is essential for document management, creating custom PDF documents, extracting specific pages, reducing file size, extracting specific content sections, and preparing PDFs for sharing or archiving. This functionality ensures you can create clean, focused PDF documents that contain only the content you need.
              </p>
            </div>
          </div>
        </section>

        {/* Why Use Extract PDF Pages Section */}
        <section className="mx-auto max-w-6xl px-4 pb-12">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-10" style={{ boxShadow: '0 12px 40px rgba(2, 6, 23, 0.08)' }}>
            <div className="inline-flex items-center gap-2 mb-4">
              <div className="h-1 w-8 bg-gradient-to-r from-emerald-500 to-green-500 rounded-full"></div>
              <h2 className="text-3xl font-bold text-slate-900">Why Use Extract PDF Pages?</h2>
            </div>
            
            <p className="text-lg text-slate-600 mb-6 leading-relaxed">
              Extracting pages from PDF documents offers numerous benefits:
            </p>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed">
                      Extract selected pages, blank pages, or pages with errors from your PDF documents. Create new PDFs containing only the pages you need and keeping only the pages you need for your specific use case.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="group p-6 rounded-2xl border-2 border-slate-200 bg-gradient-to-br from-white to-slate-50 hover:border-blue-300 transition-all duration-300 hover:shadow-lg">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed">
                      Extract pages containing specific or confidential information before sharing PDF documents. Remove personal data, financial information, or proprietary content to ensure privacy and security when sharing documents with others.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section id="how" className="mx-auto max-w-6xl px-4 pb-12">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-10" style={{ boxShadow: '0 12px 40px rgba(2, 6, 23, 0.08)' }}>
            ...
            <p className="text-lg text-slate-600 mb-6 leading-relaxed">
              Our Extract PDF Pages tool makes it easy to extract pages from PDF files:
            </p>
        ...
        <section className="mx-auto max-w-6xl px-4 pb-12">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-10" style={{ boxShadow: '0 12px 40px rgba(2, 6, 23, 0.08)' }}>
            <div className="inline-flex items-center gap-2 mb-4">
              <div className="h-1 w-8 bg-gradient-to-r from-emerald-500 to-green-500 rounded-full"></div>
              <h2 className="text-3xl font-bold text-slate-900">Best Practices for Extracting PDF Pages</h2>
            </div>
            
            <p className="text-lg text-slate-600 mb-6 leading-relaxed">
              Following these best practices ensures optimal results when extracting PDF pages:
            </p>
            ...
            <div className="space-y-6">
              <div className="p-6 rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-green-50">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-slate-900 mb-2">Preview Before Extracting</h3>
                    <p className="text-sm text-slate-700 leading-relaxed mb-3">
                      Use the PDF preview feature to review pages before extraction. Navigate through all pages to ensure you're extracting the correct pages. Verify page numbers match your intended extraction list to avoid missing important pages by mistake.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl border-2 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed mb-3">
                      Always keep backup copies of your original PDF files before deleting pages. While the tool creates new PDFs with pages extracted, preserving originals ensures you can revert changes or create different versions with different pages deleted if needed.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl border-2 border-orange-200 bg-gradient-to-r from-orange-50 to-red-50">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-slate-900 mb-2">Verify Extraction Results</h3>
                    <p className="text-sm text-slate-700 leading-relaxed mb-3">
                      After extraction, review the statistics shown for each processed file. Check that the number of extracted pages matches your expectations. Download and open the resulting PDF to verify that the correct pages were extracted and extracted pages are in the correct order.
                    </p>
                  </div>
                </div>
              </div>
